...
export_data() {
    ...
    mysql -u root -p -p -e "$select_query" "$db_name" | sed 's/\t/","/g;s/^/"/;s/$/"/;s/\n//g' >"$output_csv"
...
import_data() {
    ...
    tail -n +2 "$input_csv" | while IFS=',' read -r -a row; do
        ...
        mysql -u root -p "$db_name" -e "INSERT INTO $table_name ($column_names) VALUES ($values);" || {
...
clean_null_values() {
    ...
    for pair in "${CLEAN_PAIRS[@]}"; do
        ...
        mysql -h localhost -u root -p -p"$MYSQL_PASSWORD" "$DB" -e "$clean_query"
        if [ $? -eq 0 ]; then
...
verify_null_values() {
    ...
    columns=$(mysql -u root -N -B -e "SHOW COLUMNS FROM $table_name" "$db_name" | awk '{print $1}')
    ...
    for column in $columns; do
        (
            ...
            null_count=$(mysql -u root -N -B -e "SELECT COUNT(*) FROM $table_name WHERE $column IS NULL" "$db_name")
...
create_database() {
    ...
    existing_db=$(mysql -u root -p -e "SHOW DATABASES LIKE '$db_name'" | grep "$db_name")
...
create_database_clone() {
    ...
    existing_db=$(mysql -u root -p -e "SHOW DATABASES LIKE '$source_db'" | grep "$source_db")
    if [ -z "$existing_db" ]; then
        echo "Source database '$source_db' does not exist."
        exit 1
    fi
    # Check if the clone database already exists
    existing_clone_db=$(mysql -u root -p -e "SHOW DATABASES LIKE '$clone_db'" | grep "$clone_db")
    ...
    mysql -u root -p "$clone_db" <dump.sql
    rm dump.sql
    if [ $? -eq 0 ]; then
